...
1. 把照片放进 `source/img/gallery/photos/` 目录
2. 运行 `npm run gallery` — 自动生成缩略图（500px）、显示图（1920px）、页面数据
3. `npx hexo generate && npx hexo deploy` — 发布

### 相册目录结构

```
source/img/gallery/
├── photos/     ← 放原图（已 gitignore）
├── thumb/      ← 自动生成的缩略图（已 gitignore）
└── display/    ← 自动生成的显示图（已 gitignore）
```
